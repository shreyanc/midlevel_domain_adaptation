...
from helpers.train_funcs import test, train_da_backprop
...
from models.cpresnet import CPResnet_BackProp
...
def main():
    ...
    sc_tr_dataset = MidlevelDataset(select_song_ids=sc_tr_ids, augment=None)
    sc_va_dataset = MidlevelDataset(select_song_ids=sc_va_ids)
    sc_te_dataset = MidlevelDataset(select_song_ids=sc_te_ids)
    tg_te_dataset = MidlevelDataset(select_song_ids=tg_ids)
    ...
    tg_tr_dataset = get_extra_dataset('maestro', test_size=20000, augment=None, normalizing_dset='midlevel')[1]
    ...
    for epoch in range(1, 100):
        ...
        train_da_backprop(net, sc_tg_tr_dataloader, optimizer, criterion, epoch, writer, run_name + f'_epoch-{epoch}', dataloader_len=min(len(sc_tr_dataloader), len(tg_tr_dataloader)))
        scheduler.step()
        sc_val_corr = test(net, sc_va_dataloader, criterion, writer, epoch, mets=[test_metric])[0][test_metric]
        sc_test_corr = test(net, sc_te_dataloader, criterion, writer, epoch, mets=[test_metric])[0]
        tg_test_corr = test(net, tg_te_dataloader, criterion, writer, epoch, mets=[test_metric])[0]

        logger.info(
            f"Epoch {epoch} sc val {test_metric} = {round(sc_val_corr, 4)}, sc test {test_metric} = {round(sc_test_corr[test_metric], 4)}, "
            f"tg test {test_metric} = {round(tg_test_corr[test_metric], 4)}")
    ...
    tg_test_corr = test(net, tg_te_dataloader, criterion, writer, epoch=-1, mets=[test_metric])[0][test_metric]
